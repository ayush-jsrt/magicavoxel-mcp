...
def test_fill_box_exact_region():
    ...
    count, coords = fill_box(buf, (1, 1, 1), (2, 2, 2), 7)
    assert count == 2 * 2 * 2
...
def test_fill_box_clips_to_buffer_bounds():
    ...
    count, coords = fill_box(buf, (-5, -5, -5), (100, 100, 100), 1)
    assert count == 3 * 3 * 3
...
def test_fill_box_handles_reversed_corners():
    buf = VoxelBuffer(5, 5, 5)
    count, coords = fill_box(buf, (3, 3, 3), (1, 1, 1), 9)
...
def test_fill_box_out_of_bounds_returns_empty_coords():
    buf = VoxelBuffer(3, 3, 3)
    count, coords = fill_box(buf, (100, 100, 100), (200, 200, 200), 1)
    assert count == 0
...
def test_fill_sphere_paints_center_and_respects_radius():
    ...
    count, coords = fill_sphere(buf, (10, 10, 10), 5, 3)
    assert count > 0
...
def test_fill_sphere_out_of_bounds_is_noop():
    buf = VoxelBuffer(5, 5, 5)
    count, coords = fill_sphere(buf, (1000, 1000, 1000), 2, 1)
    assert count == 0
...
def test_fill_cylinder_along_z_axis():
    ...
    count, coords = fill_cylinder(buf, (5, 5, 5), radius=3, height=6, axis="z", color_index=2)
    assert count > 0
